...
suggested_tier: mid
neighbours: |
  before: architect/import-external-design (the design doc is authored/finalised)
  after: deliver/describe-phases-releases-waves (plan the delivery lifecycle)
---
...
- At **handover**, when an engineer submits an as-built / "what we actually shipped"
  document and you want to close the loop against the design.
...
These are the only four observations:
...
The method has a **deterministic base** (Steps 1–4, runnable with pure string/set
operations — no markdown library, no network) and a **model reasoning step**
...
```
Reconcile an AS-BUILT document (markdown describing what was actually delivered)
against the AS-DESIGNED solution-design sections, the requirements, and their
acceptance criteria. Emit neutral OBSERVATIONS — never a pass/fail verdict.

Produce four kinds of observation, and only these four:
