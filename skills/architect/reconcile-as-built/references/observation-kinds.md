...
These are observations, not verdicts. The skill names the divergences; a human
dispositions them. Nothing here ever FAILs a build.
...
## Deterministic thresholds (the base)
...
When a model is available, the model step does the same reconciliation with judgement
