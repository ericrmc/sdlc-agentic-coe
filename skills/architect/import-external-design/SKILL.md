...
neighbours: |
  before: architect/reconcile-design-vs-requirements — the design has been reconciled against requirements.
  after: architect/reconcile-as-built — once adopted, the design is checked against what was actually built.
---

# import-external-design

Merge an externally-authored solution design — written in whatever structure its author chose — onto **the canonical frozen-8 sections** that hold the single source of truth. Act as an **editor**, not a co-author: preserve what is already there, integrate the new material faithfully, dedupe overlap, thread requirement keys inline, and never invent scope. The genuinely-unmappable goes into an `unmapped` bucket — never force-fit.

The author works *elsewhere* (another IDE, a partner team, another agent); this is how their work lands on the canonical sections in-voice, with a reviewable diff, instead of being pasted over the top.

## Purpose

An externally-authored design arrives as one or more markdown files with arbitrary headings. A sectioned design already exists under a **fixed** set of canonical keys (the "frozen-8"). This skill maps and merges the incoming material into the correct canonical sections so it becomes authoritative — **faithfully, never inventing scope** — and produces a per-section **candidate-vs-current diff** to review *before* anything is adopted.

Truth is only written when the diff is adopted. The incoming bundle is **input, never truth**.

## The frozen-8 canonical sections

Target only these eight `section_key` values. Never invent a section.
...
- This is **advisory**: it produces a reviewable proposal, never an automatic write. The human decides what lands — propose then ratify, where adopting the diff is the only ratify step. There is no approval workflow to satisfy — just a clean diff and an adopt action.
