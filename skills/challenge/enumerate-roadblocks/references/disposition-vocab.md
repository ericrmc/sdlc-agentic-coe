...
> The enumerate step **enumerates and cites** roadblocks. It assigns **no status and no severity**. Everything in this file is the **human's** to apply. A disposition only *flags drift* — it never silently re-opens or closes anything downstream; a human ratifies whether a change re-opens a prior decision.
>
> Dispositions are **advisory.** No disposition freezes a workflow state, demands sign-off, or blocks the pipeline. The vocabulary exists so a human can record a deliberate, citable call against a cited constraint.
