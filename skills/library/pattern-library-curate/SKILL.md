...
one_liner: Keep the pattern shelf healthy by reading computed facts.
aliases: [library cleanup, prune patterns, stale pattern check, deprecate a pattern, sunset review, maturity audit, pattern health]
suggested_tier: mid
neighbours:
  before: library/author-capability — needs reach the shelf as patterns/capabilities to curate.
  after: library/portfolio-phase-health — derived-on-read health facts across the portfolio.
---

# pattern-library-curate — lifecycle health by reading computed facts

Keep the pattern library's shelf healthy: maturity labels honest, dates current,
supersede chains coherent, and non-adoption visible. Do this by reading what the
Actions already computed — never by asserting a number, a verdict, or a maturity.

> A curator reads the catalogue; it does not rewrite the books. Maturity is a
...
> a curator sets. Surface what's true and **propose** what to do about it. The
> human disposes.

## Purpose

Maintain the lifecycle health of the pattern library by **reading the facts the
pattern-lifecycle Action computed** — never asserting them — and turning those
facts into a small set of concrete, reviewable curation proposals:
...
- **Maturity is computed, not asserted.** `battle-tested | emerging | experimental`
  ...
  lifecycle Action, materialised in `generated/pattern-maturity.json`. **Read**
  that file. Never label a pattern's maturity from judgement.
- **Flag what's gone stale.** Patterns past their `sunset_at`, or whose
  `validity_check_months` window since `approved_at` (or `valid_from`) has elapsed,
  are surfaced for revalidation. (The Action opens revalidation issues; this skill
  reads the flags and proposes the human-facing curation.)
- **Manage supersede chains — never delete a pattern with adoptions.** When a
  pattern is replaced, **deprecate** it (`approval_status: deprecated`) and point
  `superseded_by` at the successor. A pattern with rows in the ledger is the
  historical record of real engagements; deleting it erases provenance. Propose
  the chain edit; never remove the file.
...
Everything here is **advisory**. Output is PRs (chain / `approval_status` edits) and
issues (revalidation, zero-adoption review). A human reviews and merges. Nothing
produced here blocks anything.

## When to use

- Running a periodic library health pass, or after the pattern-lifecycle Action
  has run and refreshed `generated/pattern-maturity.json`.
- A pattern has clearly been replaced and the supersede chain needs wiring up
  correctly **without** losing the predecessor's adoption history.
- An honest, current picture is wanted: which patterns are battle-tested, which
...
`architect/recommend-component-patterns`. Do **not** use it to author or validate a
new pattern — that's `library/author-component-pattern`, the PR-reviewed
contribution flow where a human attaches evidence and ratifies validity. This skill
**reads and curates** what already exists; it never mints maturity, validity, or
approval.
...
1. **`generated/pattern-maturity.json`** — the lifecycle Action's output: per
   ...
   This is the source of truth for maturity and counts. Expected shape:
...
2. **The pattern files** under `patterns/` — each with YAML frontmatter carrying
   the **human-owned lifecycle fields**: `approval_status` (`candidate|provisional|
   approved|deprecated`), `approved_by`, `approved_at`, `valid_from`,
   `validity_check_months`, `sunset_at`, `supersedes`, `superseded_by`, `evidence`.
   Scan these dates and chain pointers. **Read** them; never author `approval_status`
   / `approved_by` / `approved_at` or set a maturity into frontmatter.
...
3. **`adoptions/ledger.jsonl`** (optional, for grounding) — the append-only record
   ...
   `overridden-out`/`rejected` reason; never to re-derive the count (the Action
   owns that).
...
The base is **deterministic read first, model proposal second**. Never compute
maturity, never set validity, never delete. Read computed facts, then draft
...
1. **Read `generated/pattern-maturity.json`.** For each pattern record, take the
   **already-computed** `maturity`, `adopted_count`, the per-disposition
   ...
   0. Read the result; don't apply the rule.)
2. **Scan each pattern file's frontmatter** for the lifecycle dates and chain
   pointers: `approval_status`, `approved_at`, `valid_from`, `sunset_at`,
...
3. **Derive the flags deterministically** (date arithmetic only, against today):
   - **past-sunset:** `sunset_at` is in the past.
   - **review-due (computed):** `approved_at` (or `valid_from` if not yet approved)
     plus `validity_check_months` months is in the past — the validity window has
     elapsed. There is no stored review-by date; it is derived on read.
   ...
   - **broken chain:** a `superseded_by` that points at a `pattern_key` with no
     ...
     `approval_status: deprecated` whose `superseded_by` is empty; a successor
     that doesn't back-reference via `supersedes`.

This step is mechanical and reproducible — it is the deterministic fallback in
full. If only this can run, it still produces a correct **health table** of which
patterns are stale, zero-adopted, or chain-broken.

### STEP 2 (MODEL) — Propose curation actions as drafts (deprecate / supersede / revalidate)

Now reason over the STEP-1 facts and draft a **small** set of curation proposals.
This is a proposal; the human disposes. For each flagged pattern, choose the
fitting curation:
...
- **Revalidation (issue draft).** For `review-due` / `past-sunset` patterns: draft
  ...
  or to sunset it. Cite the exact dates that tripped the flag (the `approved_at` /
  `valid_from` + `validity_check_months` window, or the `sunset_at`). Do **not** set
  `approved_at` — revalidation is a human ratification with evidence.
- **Supersede chain (PR draft).** When a pattern is clearly replaced by a newer
  ...
  `approval_status: deprecated` and `superseded_by: <successor-key>`; on
...
can verify it in one glance. **Never** assert a maturity, set a validity field, or
delete a file — those are read or handed to a human, never authored here.
...
Nothing is applied here. A reviewer merges the PR or actions the issue. The library
stays human-governed and PR-reviewed end to end — light and advisory.
...
| pattern_key | maturity (computed) | adopted | dispositions | review-due (computed) | sunset_at | flags |
|---|---|---|---|---|---|---|
...
```
**Why:** `legacy-soap-gateway` is past `sunset_at: 2026-01-01` and the gateway/BFF
...
**Body:** The validity window has elapsed: `approved_at: 2024-12-01` +
`validity_check_months: 12` puts the computed review-due at 2025-12-01, now in the
past. Owner: please re-confirm validity with fresh evidence (a recent engagement
that used it as built), or propose sunset. *No field is changed by this issue —
revalidation is a human ratification with evidence attached.*
...
### Nothing-to-do
- `event-driven-microservices` — battle-tested (5 clean), computed review-due
  2026-09-14 (in date), chain coherent. Healthy; no action.
```
...
- **Read maturity; never assert it.** `battle-tested|emerging|experimental` comes
  ...
  ledger. If you ever find yourself *deciding* a maturity, stop — that is no longer
  the curator's role. Quote the computed value and the count it came from.
...
- **Absence is data; show adopted-by-0 honestly.** Zero-adoption is surfaced, not
  ...
  *why teams declined* is signal that sits next to the adoption tally, never
  suppressed.
- **Validity is not set here.** `approval_status`, `approved_by`, `approved_at` are
  human-owned, evidence-backed, PR-reviewed; review-due is computed on read from
  `approved_at`/`valid_from` + `validity_check_months`, not stored. Revalidation is
  an **issue drafted** for the owner — never a field written here. (An agent-set
  `approval_status`/`approved_by`/`approved_at` fails the lint.)
- **Propose, don't dispose.** Every output is a PR draft or an issue draft. A human
  reviews and merges. Nothing here blocks the library or any downstream project —
  it's a curator's advisory, light by design.
- **Cite the computed fact.** Each proposal points at the exact thing that
  ...
  proposal a reviewer can't verify in one glance is a proposal reasoned too far
  past the facts.
...
`sunset_at` / computed-review-due / `validity_check_months` semantics, and the
