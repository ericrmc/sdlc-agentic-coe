...
> This is the one canonical statement of the rhythm that every stage skill follows.
> Stage skills **quote** this file rather than re-describing it, so the wording cannot drift.
> If you are editing the rhythm, edit it **here** and let the quotes follow.

## The rhythm

Every stage in this Centre of Excellence follows the same shape, end to end:

```
human supplies / asks  ->  an AGENT does work and PROPOSES  ->  a human ratifies / edits / overrides  ->  proceed
```

- The **human supplies** the input the stage needs (a vision, a draft, a codebase pointer, a decision question).
...
This is advisory, not enforced. Nothing in this repo blocks you from moving on. There is no state machine, no governance disposition, no version-bound gate. The rhythm is a habit the skills make easy to follow, not a wall.

## The one invariant worth keeping

**The agent never ratifies its own work.**

An agent may propose, draft, recommend, estimate, flag, and red-team. A human accepts, edits, overrides, or rejects. That separation is the whole point — it is what keeps a person in control of the project's direction while letting the agent do the heavy lifting. Everything else here is light and advisory; this one line is the load-bearing principle. (It is the lifted-out core of the original app's "an agent never crosses a ratification gate" rule.)

## Advisory checkpoint wording

Where a stage skill wants to mark a natural pause — a point where a person really should look before the project moves on — it states it in exactly this advisory form:

> **Checkpoint (advisory): a human should confirm _X_ before proceeding.**

Concrete examples a stage skill might emit:
...
These are prompts, not gates. They never return an error, never set a status, never refuse to continue. They are a nudge to look, owned by a person who can choose to proceed anyway.

## PR-merge IS the ratify

In this GitHub-native Centre of Excellence, **the pull request is the ratification surface.** When an agent produces work, it lands as a proposal in a branch / PR. The human review and the **merge of that PR is the ratify step** — that is where a person accepts, edits, or overrides the proposal, in the open, with history.

This maps the rhythm onto GitHub mechanics with no bespoke machinery:
...
- It is **not** a state machine. There is no enforced ordering, no "you cannot reach stage N+1 until stage N passed."
- It is **not** an approval gate. Nothing blocks. A checkpoint is advisory; a person can proceed past an unread checkpoint and own that.
- It is **not** governance disposition. There is no `pass` / `pass_with_conditions` / `send_back` machinery. Red-teaming and panel findings are *inputs a human weighs*, not verdicts that hold the project.
- It is **not** version-bound. Stages are revisitable freely; re-running a skill simply produces a fresh proposal to ratify by merge.
