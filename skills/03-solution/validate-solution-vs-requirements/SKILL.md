...
> FORGE Stage 6 (Validation). The solution shape is chosen; now run it **back** against
> the requirements and **show only the friction.** Satisfied requirements settle and stay
> quiet. The compromises surface — each one walking the trace edge *up* to the business
> outcome and capability it dents. You are the **validator**: you compute flags, you never
> emit a verdict.

## Purpose

A solution has been adopted — a pattern, or a narrowed-down shape from the multi-solution
explore. It carries **enforced constraints**: the things it makes you give up. *"All writes
are async via the event bus; no synchronous DB writes."* *"Single region only."* Those
constraints are real, and some of your requirements will collide with them.
...
1. **Nothing is dropped without flagging.** Every compromise walks the trace edge *up* to
   ...
   **visible**, never quietly tidied away. (This is the honesty contract: the old dossier
   filtered to "accepted" and dropped every compromise; this skill does the opposite — it
   leads with them.)
...
There is **no gate** here. A validation pass that finds zero compromises is a legitimate,
non-failing outcome — it means the solution fits. The output is friction to rule on, not a
pass/fail stamp.

## When to use

- A **solution shape is chosen** (a pattern adopted in Stage 4, or a shape narrowed in the
  Stage 5 explore) and you need to validate it back against the requirements.
...
Pairs with **recommend-components / adopt-pattern** (which supplies the enforced constraints)
upstream, and with **necessity-check** (Stage 7) downstream — validation finds where the
solution falls *short* of a requirement; necessity finds where a component *over-reaches* a
requirement. Both walk the same trace edge.
...
2. **The requirement set** — the live derived requirements, each with:
   ...
   - its `capability_tags` — what the requirement *needs* (`instant-write`, `cross-region`,
     `real-time`, …). If tags are absent, infer them in the LLM step and **say you inferred them.**
...
If you have no `req_key`s, say so and emit the deterministic replay table with empty keys —
do not invent ids.

## The method (STEPS)

The spine is **deterministic** (the replay table); the LLM step only **phrases** the
...
This is the deterministic fallback and the spine of the whole skill. For **every** requirement,
...
reproducible, provider-independent floor: even with no model available, you return every
requirement with its computed flag and the constraint key it clashed on.

### Step 2 — Keep only the friction; settle the rest

A requirement flagged `met` **settles** — it is reported as a count, not as a card. The screen
(and the output) leads with the **`compromised` and `unmet`** rows only. This is "show only the
friction": satisfied requirements are quiet; compromises are loud.
...
### Step 4 — LLM STEP: phrase each compromise as a grounded question
...
> You are the SOLUTION VALIDATOR. You COMPUTE FLAGS, NEVER VERDICTS. The `met`/`compromised`/
> `unmet` flags were already computed deterministically from the constraint replay — you do not
> change them, re-rank them, or add a "looks fine / looks risky" assessment of your own.
>
...
kind `compromise_accept` / `compromise_reject`, and an entry in `OPEN-QUESTIONS.md`). You compute
the flag; the human owns the call.

## Output format

Lead with the computed summary, then the **replay table** (the deterministic spine), then the
...
### Constraint replay (the deterministic spine)
...
- **Nothing dropped without flagging.** Never quietly filter out a compromise to make the solution
  ...
  The old failure mode was a dossier that filtered to "accepted" and hid every trade-off — this
  skill is the antidote.
...
- **Advisory, not a gate.** A pile of `unmet` requirements does not *block* the project. It hands
