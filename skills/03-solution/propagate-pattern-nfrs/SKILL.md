...
> Carry an adopted pattern's governed quality bar into your project's requirements — for free, and faithfully.

## Purpose

A vetted component pattern is not just a recipe for *what* to build — it carries a
**governed quality bar**: the non-functional requirements (NFRs) that the people who
proved the pattern decided it must always meet. Encryption at rest. p95 latency under
200ms. Exactly-once sync. WCAG 2.2 AA. Those NFRs are the *evidence* that the pattern is
production-grade, and they live in the pattern's `attached_nfrs` frontmatter.

The whole point of a Centre of Excellence is that **adopting the pattern should pull its
quality bar in with it.** You should not have to re-derive "this must be encrypted" by
hand every time you reuse the encrypted-store pattern — the pattern already knows. This
skill does that propagation: when a delivery team adopts a pattern, each attached NFR is
placed against the **business outcome it most directly serves** and emitted as a
**derived non-functional requirement**, traced back to its parent outcome.

The discipline that makes this trustworthy is **faithfulness**. You are a requirements
*tracer*, not a requirements *author*. You decide only **where each NFR hangs** — you
never decide *whether* it applies, never reword it, never merge two into one, never
invent a new one. The pattern's quality bar arrives intact or it does not arrive at all.
...
```
You are a requirements tracer. A delivery team has adopted a reusable
architecture/governance PATTERN. That pattern carries non-functional requirements
(NFRs) that must now be applied to the current project and, where possible, traced to
the specific business outcome each one best serves.
...
- **This is a doc-emit, not a database write.** The original app wrote these derived NFR
  rows directly into the project on pattern-accept (a `pattern_nfrs_propagated` event +
  `insert_derived_requirement` per NFR). **That on-accept DB write is DROPPED.** Here the
  output is a **proposal** — markdown lines the human reviews and folds into
  `REQUIREMENTS.md`. No state machine, no auto-apply, no gate. Light and advisory.
- **No orphan-by-fabrication.** The old code, when no outcome matched, fell back to
  attaching the NFR to the *first* accepted outcome. **Do not do that.** A forced parent
  is a lie about traceability. The honest fallback is **pattern-level** (`parent_key:
  null`) — surface it as such; never invent a link to make the trace look complete.
