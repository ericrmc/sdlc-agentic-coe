...
deterministic_fallback: the brand-baseline + surfaces/deliverables spine + studio prompt
...
Scaffold a clear, paste-ready **design & branding brief** for an external design studio (or design agent), so the people who do the professional design work get everything they need in one document — and nothing they don't.

This composes the **scaffold-then-handoff** philosophy: *you* assemble the brief, the *studio* does the professional design. Be explicit about the boundary:

- **The studio does the design.** Style guide, design tokens, key-surface mockups — that is their craft, not yours. You are not rendering anything.
- **The brief is a scaffold, not a deliverable.** It is a believable, project-aware starting point that the studio sharpens. You lead with open questions rather than pretending the design is decided.
- **Captured client brand is a FIXED constraint.** If the client already owns a brand (logo, palette, type, voice), you capture it once into a lean *brand baseline* and treat it as a non-negotiable input. The studio **reuses** it; nobody **re-derives** it. Greenfield brands are the studio's to invent — owned brands are theirs to honour.
- **Returned assets re-enter as references, not binaries.** When the studio hands back a style guide / token file / mockups, you point at them (repo links, design-tool URLs, file references). You do not paste images into the brief or store binaries; the brief stays a lean pointer document.

Keep it light and advisory. There is no approval gate here — the brief is a clear ask, not a contract you enforce.

## When to use

Use this skill when you are about to hand UI, visual design, or branding to an **external** design studio or a design agent, and you want a single paste-ready brief that:

- captures the client's white-label brand once and freezes it,
- states the surfaces and deliverables you expect back,
- carries a fenced **studio prompt** you can copy straight into the studio's intake (or into a design agent),
- and routes returned assets back in as references.

Do **not** use it to do the design yourself, to produce mockups, or to gate/approve the studio's work. This is the handoff, not the studio.
...
You provide these as markdown or a short context blob. The skill reads what is relevant and ignores the rest.

## The method (steps)

The method has a **deterministic spine** (Steps 1–4) that always runs and always produces a usable brief, and an **LLM enrich step** (Step 5) that sharpens the prose without changing the facts. This mirrors the source flow: the platform assembles a deterministic scaffold, then an LLM enriches it and falls back to the scaffold on any error.

### Step 1 — Lead with open questions (deterministic)

Open the brief with the questions the studio must answer before it can start. This is house style: surface the unknowns first, do not bury them. Generate these from what is **missing or ambiguous** in the inputs, e.g.:
...
The baseline is captured **once**. On regeneration you reuse it verbatim — you never re-derive or "improve" a client-owned brand.

### Step 3 — Project the surfaces & deliverables spine (deterministic)

Render the body spine that does not depend on the LLM:
...
### Step 5 — LLM enrich pass (reasoning step, keeps the brand baseline)
...
If anything goes wrong in this step — missing model, error, garbled output — **fall back to the deterministic scaffold from Steps 1–4**. The scaffold alone is a complete, shippable brief. The LLM only makes it sharper, never load-bearing.
...
- **Don't re-derive an owned brand.** A captured brand baseline is a fixed constraint. "Improving" or regenerating a client's palette/type/logo is the most common failure mode — reuse it verbatim.
...
- **Don't gate.** This is advisory. The brief is a clear, paste-ready ask — not an approval you enforce against the studio's output.
