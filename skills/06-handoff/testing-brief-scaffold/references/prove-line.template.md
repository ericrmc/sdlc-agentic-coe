...
<!--
  THE SPINE. Walk accepted outcomes -> derived requirements -> acceptance
  criteria. Emit exactly one `PROVE:` line per acceptance criterion. Preserve
  every key, type, and text verbatim — traceability depends on it.
...
<!--
  ...
  synthesised. Fold its synthesis in verbatim. If no panel was convened, omit
