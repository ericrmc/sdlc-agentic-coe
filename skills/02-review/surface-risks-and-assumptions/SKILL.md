...
description: Maintain a light markdown RAID register — surface the small set of load-bearing assumptions the plan depends on, deduped against the existing register (open and closed); agent proposes open, human disposes. Use this to ground a project's hidden dependencies — the advisory replacement for risk governance gates.
when_to_use: grounding a project's hidden dependencies before/while you build; the advisory replacement for risk governance
...
A RAID register is the small, honest list of things your plan is *betting on*. Most of those bets are invisible: nobody wrote them down because they felt obvious at the time. This skill drags the load-bearing ones into the light so that, when one turns out to be wrong, you find out on purpose instead of by accident.

It is **advisory, not a gate.** Nothing here blocks a phase, freezes a state machine, or demands a sign-off. The register sits beside the work and grounds it. The only discipline it enforces is a division of labour: **the agent proposes, the human disposes.**
...
This is the advisory replacement for heavyweight risk governance. There is no disposition state-machine, no version-bound gate, no approval ceremony. There is a markdown file, a derive step, and a human who closes items out.
...
The method has a **deterministic spine** (the register file and an exact-match dedupe you can run without any model) and one **LLM reasoning step** (the derive prompt). Keep them distinct: the model is the semantic deduper; the deterministic backstop only stops a verbatim repeat slipping through.
...
> Advisory, not a gate. Agent proposes `open`; a human disposes (validated/invalidated,
...
- **Advisory, never a gate.** If you find yourself blocking a phase on a register item, stop — promote it to a roadblock or just flag it. The register grounds; it does not enforce.
