...
# author-component-pattern — write one pattern file, let the PR ratify it

> A pattern is a *promise the library makes to the next project*: "this shape has
> been built, here's the evidence, here are the quality bars it carries, and here
> is exactly when it stops being true." This skill writes that promise as one
> markdown file. It does **not** make the promise binding — the **pattern PR
> review** (CODEOWNERS + branch protection) is where a human turns a candidate
> into something the library will recommend.
...
sets `approved`. Ratification is a human act performed through the **pattern PR
review**: a CODEOWNER reviews the diff, confirms the evidence is real, and merges
— and only then, in a follow-up edit the human owns, does the status climb. The
frontmatter carries the full lifecycle/provenance field set (dates, validity
check, sunset/supersede, attached NFRs with acceptance criteria, evidence) so the
reviewer has everything they need *in the diff*. See `CONTRIBUTING.md` for the
review flow and CODEOWNERS map.

This is light and advisory. A pattern is a recommendation with a track record, not
a mandated component and not a gate. A downstream project may adopt it, override it
out (with a reason), or ignore it. The pattern's job is to be honest enough that
...
- You are **promoting an explored solution** (the output of
  `surface-solution-options` / a bespoke design that proved itself) into a library
  candidate through the PR-reviewed contribution flow.

Do **not** use this to:
- *Recommend* an existing pattern → that is `recommend-component-patterns`.
- *Propagate* an adopted pattern's NFRs into a project → `propagate-pattern-nfrs`.
...
- Mint a pattern "on the fly" mid-design with no evidence it was ever built — a
  ...
  `surface-solution-options`, not the library.
...
2. **The category** — one of `deployment | integration | data` (the closed enum;
   matches the six seeded patterns).
...
4. **The evidence it was built** — links/artefacts (a repo, a PR, a runbook, a
   design doc, a deployed-service URL). Required only once a human is promoting it
   past `candidate`; the agent records what it is given and leaves the slot honest
   if there is none yet.

You do not need a database or a network call. The pattern is a plain markdown file
in a git repo; the schema validation runs in CI on the PR.
...
| field | type | notes |
|---|---|---|
| `pattern_key` | string | stable, cite-able, UPPER-KEBAB after `PAT-` — `PAT-WEBAPP-PG` (matches `^PAT-[A-Z0-9]+(-[A-Z0-9]+)*$`). Survives renames; the thing `recommend-component-patterns` and the build bundle cite. The **filename** is a separate human-readable lower-kebab slug and need NOT equal the key. |
...
| `attached_nfrs` | list | each `{ kind, statement, acceptance_criterion }`; `kind` from the closed 11-value enum (`nfrs/nfr-kinds.md`). The governed quality bar `propagate-pattern-nfrs` carries into adopting projects. |
...
| `constraints` | list of `{ statement, enforced }` | — | what the pattern makes you give up; `enforced` ∈ `hard` (cannot be waived) \| `soft` (waiving it is a recorded compromise). Fuel for `validate-solution-vs-requirements`. |
...
The 11 values, from `nfrs/nfr-kinds.md` (mirrors the seeded patterns):
...
## Seeded from the existing patterns

The library ships with three converted worked examples under `patterns/` — read them
as the gold standard for tone, granularity, and honest trade-offs before you author:
...
Each one names its trade-off in the body (the gateway as a SPOF; the single DB write
authority; UC platform lock-in) — none pretends the shape is free. Match that honesty.
(More seeds — an event broker, a serverless API, an on-prem VM cluster — are queued for
conversion; see `patterns/README.md`.)

## The method — STEPS

### STEP 1 (DETERMINISTIC) — Lay down the frontmatter template + body skeleton

This is the mechanical spine; it runs with no model. Coin `pattern_key` as a stable
...
This is the judgement step, and the only one that needs a model. From the user's
chosen solution, draft:
...
```
You are a pattern author on an internal Centre of Excellence. A delivery team built
a solution worth reusing. Write the SUMMARY and TRADE-OFFS for a library pattern
capturing it, plus a testable acceptance_criterion for each governed NFR.
...
### STEP 3 — Hand off to the pattern PR review (do not self-approve)

The file is now a **candidate**. Open (or instruct the human to open) a PR. CI runs
the frontmatter schema validation (`pattern.frontmatter.schema.json`) and the
stale/sunset checks. A CODEOWNER reviews the diff, confirms the **evidence is real**,
and merges. Only then does a human — in an edit they own — set
`approval_status: provisional`/`approved`, `approved_by`, `approved_at`, and the
`evidence` list. **You stop at `candidate`.** Point the user at `CONTRIBUTING.md`.
...
## Attached NFRs (the governed quality bar)
The four NFRs above propagate into an adopting project via `propagate-pattern-nfrs`,
...
## References
- `recommend-component-patterns` — how this gets retrieved for a project.
- `propagate-pattern-nfrs` — how its NFRs flow into an adopting project.
...
- **Name the load-bearing NFR honestly; do not oversell.** Every one of the six
  seeded patterns states its cost. A trade-offs section that reads like marketing is
  a defect. The most useful sentence in a pattern is often "do NOT adopt this when…".
...
- **`pattern_key` is forever.** It is UPPER-KEBAB (`^PAT-[A-Z0-9]+(-[A-Z0-9]+)*$`) and
  survives renames — it is what every recommendation and build bundle cites. It is
...
- **Light and advisory.** A pattern is a recommendation with a track record. There
  is no state machine, no enforced gate, no auto-apply. The PR review is a human
  quality bar, not a workflow lock.
