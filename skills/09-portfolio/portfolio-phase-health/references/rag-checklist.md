# RAG checklist — the deterministic spine for `portfolio-phase-health`
...
Carried over from the legacy derived-on-read `project_health()` (no persisted score). The phase axis
replaces the legacy workflow-state index; the bars and their `try/except`-degrades-safely discipline
are preserved.
...
Source enums (from the legacy model, for label/field mapping):
