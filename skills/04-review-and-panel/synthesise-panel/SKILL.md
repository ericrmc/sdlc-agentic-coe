...
> Facilitator's closing move. The panel argued the design with itself; this step writes the honest
> synthesis of what it surfaced and what it proposes a human now weigh. **The panel proposes; the human
> disposes.** Never imply a decision was made or scope was changed.

## Purpose

A deliberation panel produces a transcript: a set of contributions, each one persona answering one
...
The discipline that makes this trustworthy: the synthesiser is *incapable of a verdict by construction*. It
...
- **Closing a panel.** This is the natural second half of `convene-a-panel` — fold it in, or run it
...
- **The panel contributions** — for each: the persona (and its side, affirmative or adversarial), the
...
## The method (preserve the deterministic spine AND the LLM step)

### STEP 1 — Cluster the transcript by node + signal (deterministic spine)

Group the contributions by the **node** they touched and read the **signal mix** within each cluster. This
is mechanical — it counts and clusters signals, it does not parse prose — so it runs with or without an LLM.
...
- **A genuine split is not auto-resolved.** When an affirmative voice proposes X on a node and an adversarial
  voice objects on the *same* node, the synthesiser does not pick a winner. It emits a decision whose options
...
### STEP 2 — Write the synthesis (the LLM reasoning step)
...
honestly against the cited rows. Use this prompt (lifted from the source `synthesise_panel.md`, generalised
to be provider-agnostic):

> You are the **facilitator** closing a design-deliberation panel. The panel has argued the design with
> itself, each lens reading a real finding. Write the synthesis: a clear, honest narrative of what the panel
> surfaced and what it proposes a human should now weigh. **The panel proposes; the human disposes — never
> imply a decision was made or scope was changed.**
>
...
- **Light and advisory.** No gate, no state machine, no approval step. The synthesis is a source of
  proposals, not a destination that blocks anything. Each proposal re-enters the normal flow on its own home
  view, accepted or dismissed exactly as any other proposal of its kind.
