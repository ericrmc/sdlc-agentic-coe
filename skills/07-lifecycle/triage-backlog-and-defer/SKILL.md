...
3. **How much should we care?** — a priority that *falls out of* the trace, not out of enthusiasm. Traced items carry more priority; untraced flags carry less. Stay honest; do not inflate.

And critically: triage **remembers**. When a human declines an item, that decision is durable. The item is never re-proposed against unchanged evidence — the why-not is preserved. (This mirrors the dismissal-memory principle the platform applies everywhere: a record of what was decided *not* to do, and why.)
...
The method has a **deterministic spine** (a mechanical table anyone can reproduce) and an **LLM reasoning step** (judgement the table cannot do). Run the spine first; use the LLM step to refine trace and rationale.
...
The human reviews the suggested deferrals and parks the ones worth keeping. Parking resolves the outcome key to a real trace edge and records the item as `parked`. The item now lives on the backlog with its trace, size, and priority attached. Nothing is enforced; nothing is gated.
...
Decline ≠ delete. A declined item is remembered, not erased. (This is the same instinct as a dissent register: keep the rejected path on record.)

## GitHub-native mapping

Triage maps cleanly onto a **GitHub Project backlog**, so downstream teams run it with native mechanics — no bespoke app:
...
A light GitHub Action can validate that every non-`scope-creep` card carries an `outcome:` label and a non-empty estimate before it leaves the backlog — advisory (a check annotation), never a blocking gate.
...
### The triage prompt (the LLM step, verbatim-faithful)
...
- **Never gate.** This is advisory. You produce a menu; the human parks, promotes, or declines. No status machine, no approval, no blocking. The only "enforcement" is a soft, annotating CI check that *informs* — it never stops anyone.
