...
The keyword → outcome map is project-specific; below is the reference map from the source workbench. Adapt the labels to your project's context, keep them short, and return `null` (a value-orphan) when nothing matches.
...
| respond      | Responsive workbench keeps delivery leads in flow.                  |
...
The backstop is intentionally blunt. Treat any case where the deterministic pass and the LLM disagree as an edge case worth a human glance — that disagreement is exactly where the interesting requirements live.
